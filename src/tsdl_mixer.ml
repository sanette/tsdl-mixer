...
module Mixer = struct
  ...
  let from : Dl.library option =
    (if debug then
       Sdl.(
         log_info Log.category_system "Loading Sdl_mixer, Target = %s"
           Build_config.system));
  ...
  let init flags =
    ...
    if flags' = Init.empty && flags <> Init.empty then error () else Ok flags'

  let quit = foreign "Mix_Quit" (void @-> returning void)
  let mix_channels = 8
  ...
  let open_audio =
    foreign "Mix_OpenAudio"
      (int @-> int @-> int @-> int @-> returning zero_to_ok)

  let allocate_channels = foreign "Mix_AllocateChannels" (int @-> returning int)

  let query_spec =
    foreign "Mix_QuerySpec"
      (ptr int @-> ptr uint16_t @-> ptr int @-> returning int)
  ...
  let load_wav_rw =
    foreign "Mix_LoadWAV_RW"
      (rw_ops @-> int @-> returning (some_to_ok chunk_opt))

  let ( >>= ) o f = match o with Error e -> Error e | Ok a -> f a
  let load_wav file = Sdl.rw_from_file file "rb" >>= fun rw -> load_wav_rw rw 1

  let load_mus =
    foreign "Mix_LoadMUS" (string @-> returning (some_to_ok music_opt))

  let load_mus_rw =
    foreign "Mix_LoadMUS_RW"
      (rw_ops @-> int @-> returning (some_to_ok music_opt))

  let load_mus_type_rw =
    foreign "Mix_LoadMUSType_RW"
      (rw_ops @-> music_type @-> int @-> returning (some_to_ok music_opt))

  let quickload_wav =
    foreign "Mix_QuickLoad_WAV"
      (ptr uint8_t @-> returning (some_to_ok chunk_opt))

  let quickload_raw =
    foreign "Mix_QuickLoad_RAW"
      (ptr uint8_t @-> uint32_t @-> returning (some_to_ok chunk_opt))

  let free_chunk = foreign "Mix_FreeChunk" (chunk @-> returning void)
  let free_music = foreign "Mix_FreeMusic" (music @-> returning void)
  ...
  let hook_music_finished =
    foreign "Mix_HookMusicFinished"
      (funptr (void @-> returning void) @-> returning void)

  let get_music_hook_data =
    foreign "Mix_GetMusicHookData" (void @-> returning (ptr void))

  let channel_finished =
    foreign "Mix_ChannelFinished"
      (funptr (int @-> returning void) @-> returning void)
  ...
  let register_effect =
    foreign "Mix_RegisterEffect"
      (int
      @-> funptr effect_func_t
      @-> funptr effect_done_t
      @-> ptr void
      @-> returning nonzero_to_ok)

  let unregister_effect =
    foreign "Mix_UnregisterEffect"
      (int @-> funptr effect_func_t @-> returning nonzero_to_ok)
  ...
  let set_panning =
    foreign "Mix_SetPanning"
      (int @-> uint8_t @-> uint8_t @-> returning nonzero_to_ok)

  let set_position =
    foreign "Mix_SetPosition"
      (int @-> int16_t @-> uint8_t @-> returning nonzero_to_ok)
  ...
  let group_count = foreign "Mix_GroupCount" (int @-> returning int)
  let group_oldest = foreign "Mix_GroupOldest" (int @-> returning int)
  let group_newer = foreign "Mix_GroupNewer" (int @-> returning int)

  let play_channel_timed =
    foreign "Mix_PlayChannelTimed"
      (int @-> chunk @-> int @-> int @-> returning nat_to_ok)
  ...
  let fade_in_music_pos =
    foreign "Mix_FadeInMusicPos"
      (music @-> int @-> int @-> double @-> returning nat_to_ok)

  let fade_in_channel_timed =
    foreign "Mix_FadeInChannelTimed"
      (int @-> chunk @-> int @-> int @-> int @-> returning nat_to_ok)

  let fade_in_channel channel chunk loops ms =
    fade_in_channel_timed channel chunk loops ms (-1)

  let volume = foreign "Mix_Volume" (int @-> int @-> returning int)
  let volume_chunk = foreign "Mix_VolumeChunk" (chunk @-> int @-> returning int)
  let volume_music = foreign "Mix_VolumeMusic" (int @-> returning int)
  let halt_channel = foreign "Mix_HaltChannel" (int @-> returning zero_to_ok)
  let halt_group = foreign "Mix_HaltGroup" (int @-> returning zero_to_ok)
  let halt_music = foreign "Mix_HaltMusic" (void @-> returning zero_to_ok)
  ...
  let fade_out_group = foreign "Mix_FadeOutGroup" (int @-> int @-> returning int)

  let fade_out_music =
    foreign "Mix_FadeOutMusic" (int @-> returning nonzero_to_ok)

  let set_music_cmd = foreign "Mix_SetMusicCMD" (string @-> returning zero_to_ok)
  ...
  let get_sound_fonts = foreign "Mix_GetSoundFonts" (void @-> returning string)

  let each_sound_font =
    foreign "Mix_EachSoundFont"
      (funptr (string @-> ptr void @-> returning int)
      @-> ptr void
      @-> returning nonzero_to_ok)

  let fading_music = foreign "Mix_FadingMusic" (void @-> returning fading)
  let fading_channel = foreign "Mix_FadingChannel" (int @-> returning fading)
  ...
  let pause_music = foreign "Mix_PauseMusic" (void @-> returning void)
  let resume_music = foreign "Mix_ResumeMusic" (void @-> returning void)
  let rewind_music = foreign "Mix_RewindMusic" (void @-> returning void)
  ...
  let playing = foreign "Mix_Playing" (int @-> returning bool)

  let playing = function
    | Some channel -> playing channel
    | None -> playing (-1)

  let playing_music = foreign "Mix_PlayingMusic" (void @-> returning bool)

  let get_chunk =
    foreign "Mix_GetChunk" (int @-> returning (some_to_ok chunk_opt))

  let close_audio = foreign "Mix_CloseAudio" (void @-> returning void)
end
